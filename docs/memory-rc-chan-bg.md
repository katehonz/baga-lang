...
Статус: **решено A (2026-08-12)** — без имплементация на B/C в v0.x.
RC1.4 immortal handles покриват hop идиома leak-safe. B (tag-нат
payload) и C (`go` capture retain) остават изрично отложени.

Зависи от RC1–RC4 + RC1.4 (`docs/memory-rc-bg.md`). Каналите днес носят
само `i64` (`baga_chan_send/recv` в `src/codegen_c.c`). Heap стойности
минават през handle cast-ове: `str_h` / `bytes_h` / `map_h` → `i64` → `h_*`.
...
**Решение: A.** B пипа типовата система и канала; C е към struct-полета
(задача 4). Leak > корупция вече е спазено. Не имплементираме B/C,
докато някой не отвори отделен план с критерий (балансиран recv без
underflow + typed hop тест).
