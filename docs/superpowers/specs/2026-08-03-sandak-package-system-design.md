...
## Промени при имплементацията (2026-08-03, приети при ревю)

- **Поле `kind = "bin"|"lib"`** (default `lib`) в `[package]` — нужно, за да знае
  `sandak build` дали да линква бинарник или само `--lib` проверява. `entry` е
  задължителен само за `kind = "bin"` (std няма единен корен).
- **Dockerfile**: `sandak fetch && sandak build --locked` (fetch първо) —
  lock-ове с path deps не са преносими, затова при липса на lock той се генерира
  на място; комитнат lock (git-dep приложения) се валидира от `--locked`.
- **Lock**: root пакетът се записва с константен `source = "path+."` —
  преносим между машини; path *зависимостите* пазят абсолютни пътища
  (monorepo ограничение — затова `sandak.lock` е в .gitignore в монорепото).
- **Git кеш**: `.sandak/cache/<name>-<ref>/` + symlink `.sandak/cache/<name>`,
  за да се резолвират пакетните импорти през `-I` родителя на кеша.
- Парсинг навсякъде с `strtok_r`; цикъл-детекция преди diamond-skip в резолвъра.
