...
magic 8 = "BAGASST1"
u32 le count
records sorted by key (byte/string order):
  u8 op | u32 klen | key | u32 vlen | val
u32 le crc32c(body from after magic through last record)
```

Lookup: newest table first; linear scan (MVP — index/block index later).
