...
type           = "&", type                       (* reference *)
               | "[", type, "]"                  (* array *)
               | ident ;                         (* named: i32 i64 f64 bool str Vec ... *)
...
A `Vec` is a heap-allocated, growable array. Integer vectors and string
vectors use separate builtin families.

| Signature | Description |
|-----------|-------------|
| `vec_new() -> Vec` | Create an empty vector. |
| `vec_len(v: Vec) -> i64` | Number of elements. |
| `vec_push(v: Vec, x: i64)` | Append an integer. |
| `vec_get(v: Vec, i: i64) -> i64` | Read the integer at index `i`. |
| `vec_set(v: Vec, i: i64, x: i64)` | Overwrite the integer at index `i`. |
| `vec_push_str(v: Vec, s: str)` | Append a string. |
| `vec_get_str(v: Vec, i: i64) -> str` | Read the string at index `i`. |
| `vec_set_str(v: Vec, i: i64, s: str)` | Overwrite the string at index `i`. |
