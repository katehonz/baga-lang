...
| net     | tcp_listen/accept/connect/read/write/close; `tcp_connect_to` (DNS + timeouts + NODELAY/KEEPALIVE), `tcp_resolve_ipv4`, `tcp_set_timeouts`; `poll` — `poll_wait`/`poll_has` (event loop); `http_client` — HTTP/1.1 client (`http_get`/`http_post`/`http_request`, `Map<str,str>` headers, chunked) | !Net (+!IO)  |
