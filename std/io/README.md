...
- `read_line(r: Reader) -> str !IO` — one line without the trailing `"\n"` and without a trailing `"\r"` (CRLF and LF yield the same content). `""` at EOF (also `""` for an empty line — use `read_n` when the distinction matters). Reads one byte at a time: simple, correct, not fast.
