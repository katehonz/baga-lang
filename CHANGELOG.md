...
- **Гарантиран префикс.** `while true` с изход изпълнява тялото поне
  ...
  излезе) се брои и класифицира първата блокираща операция. Операции
  след изход не се броят — `send` след условен `return` не е капацитет
  (пазено от `server_nocap` в оракула).
- **`wf_maybe_loop`.** Цикълът може да се върти безкрай: join-завършване
  (**ДОКАЗАНО** „join след send"/„друг producer") и over-send проверката
  се гейтват; цикълът **ЦИКЪЛ** през loop остава звуков — гарантираният
  първи recv блокира, родителят чака join, никой не праща — **ОБРОЧЕНО**
  „join преди send" (виси и на runtime).
- **Посокова неизвестност.** Loop producer маркира неизвестност само за
  REFUTED посоки, които може да осуети (recv/select — може да произвежда
  неограничено); loop consumer — само за send-blocking/over-send REFUTED
  (може да дренира неограничено). Обратните посоки остават звукови:
  потребителят помага на блокирането, не го предотвратява.
- **Граници.** Изречения след цикъла, след `if` със завъртащ се клон и в
  `for` тяло със сървърен цикъл не са гарантирани — обхождането спира или
  е complex; `while` с не-`true` условие остава complex.
- Оракъл: `examples/verify/while_loop.baga` (6 ДОКАЗАНО „свободен слот",
  „recv след producer", 2 ОБРОЧЕНО „recv без send", цикъл през loop
  ОБРОЧЕНО; трети send към дрениращ loop и не-true while — честно
  мълчание); адверсариален калъф `lp6_server_join_bad` в
  `examples/verify/lp6_hunt.baga`.
- Докс: `docs/thesis-open-problems.md` §1.4 (M30), `docs/language-bg.md`
  §14.3.13, `docs/language-en.md` §14.3.13.

### verify — M29: вложен go със структуриран join
- **Рекурсивна класификация (дълбочина ≤ 4).** `go(child, канал)` в тялото
  на worker сканира детето рекурсивно; непроследен канал, packed аргументи,
  неизвестна функция или изчерпана дълбочина — честен complex.
- **join(h) сгъва детето**: гарантираните бройки, `imprecise` флага,
  `noreturn` (join на дете, което никога не завършва, не завършва никога —
  наследява се през произволен брой слоеве) и — ако родителят още не е
  блокирал — първата блокираща операция на детето. Цикъл през два слоя
  join е ОБРОЧЕНО („join преди send"), безкрайност през два слоя —
  ОБРОЧЕНО („join на worker без изход"); и двете висят и на runtime.
- **detach(h) и неjoin-нати деца** се сгъват само като кредит
  (`wf_child_cons`): гарантираната им консумация се брои, но завършването
  на родителя не ги чака — без send-fold, noreturn и първи op. Дете в `if`
  клон или тяло на цикъл може да се изпълни 0 или k пъти — complex;
  двоен join — complex.
- **`go_bg` консумацията** вече е пълният гарантиран брой
  (`n_recv + child_cons`), не 1 на worker — затваря подброяване от M24
  (loop consumer през `go_bg` кредитираше 1 вместо 3).
- **Фикс на M27 дупка** в over-send сметката: imprecise consumer там не
  маркираше unknown (минимум-кредитът подценява стаята → фалшиво
  ОБРОЧЕНО беше възможно).
- Оракъл: `examples/verify/nested_go.baga` (3 ДОКАЗАНО „recv след
  producer", 2 „побира се", 5 „свободен слот", цикъл и безкрайност през
  два слоя ОБРОЧЕНО); адверсариални калъфи `lp6_nested_cycle_bad`/
  `lp6_nested_inf_bad` в `examples/verify/lp6_hunt.baga`.
- Докс: `docs/thesis-open-problems.md` §1.4 (M29), `docs/language-bg.md`
  §14.3.12, `docs/language-en.md` §14.3.12.

### verify — M28: join на доказуемо безкраен worker
- **`while true` без break/return в тялото** никога не излиза — checker-ът
  е съгласен (такъв цикъл е дивергентен, return след него не се изисква,
  M19). Когато цикълът е на правата пътека в тялото на worker, worker-ът
  никога не завършва и `join` чака завинаги — **ОБРОЧЕНО** „join на
  worker без изход" (kind-3). Виси и на runtime (потвърдено).
- **Честни граници.** Цикъл в клон може да не се изпълни — no-claim;
  цикъл с изход вътре (`if v == 0 { return }`) зависи от данните —
  no-claim; `go_bg` на безкраен worker е добре (нищо не го join-ва).
  Префиксните бройки преди цикъла остават валидни.
- Оракъл: `examples/verify/worker_term.baga` (join-без-изход ОБРОЧЕНО;
  4× „свободен слот"; `join_maybe`/`join_cond` — честно мълчание;
  `go_bg` и броен цикъл — зелени); адверсариален калъф
  `lp6_join_inf_bad` в `examples/verify/lp6_hunt.baga`.
- Докс: `docs/thesis-open-problems.md` §1.4 (M28), `docs/language-bg.md`
  §14.3.11, `docs/language-en.md` §14.3.11.

### verify — M27: if/else сливане в worker scan-а (гарантирани минимуми)
- **Двата клона се сканират отделно** и бройките се сливат като
  гарантирани минимуми — точно това, което кредитите, producer
  капацитетът и over-send проверката имат нужда за ДОКАЗАНО посоките
  си. Първата блокираща операция се класифицира само когато двата клона
  съвпадат; клон, който може да не блокира (или блокира различно), преди
  да е видяна операция, прави скана complex. `while`/`match` остават
  complex — искат индукция.
- **`wf_imprecise`.** Тяло с различаващи се клонови бройки е неточно:
  минимумите продължават да доказват, но всяка ОБРОЧЕНО посока по канала
  вижда unknown (тя иска точност). Същото важи за `go_bg` (unknown) и за
  цикли, съдържащи неточно тяло.
- **Звуков фикс на M26.** select ДОКАЗАНОто вече изисква `!unk` —
  неизвестен consumer можеше да изяде елемента, а `wf_chan_eventual`
  го броеше като наличен.
- Примери: `if` с равни клонови бройки е точен producer (M25×M27
  композиция през броен цикъл); recv-first с условен send след него е
  чист кредит; 1-или-3-send producer покрива само първия recv, после
  мълчи; 2-или-4-send към cap-3 при join — честно мълчание (клонът с 4
  send-а би препълнил буфера; фалшиво ДОКАЗАНО е затворено).
- Оракъл: `examples/verify/branch_if.baga` (5 ДОКАЗАНО „recv след
  producer", 3 „побира се", 2 ОБРОЧЕНО — over-send-ът виси и на
  runtime); адверсариални калъфи `lp6_if_short_bad`/`lp6_if_over_bad` в
  `examples/verify/lp6_hunt.baga`.
- Докс: `docs/thesis-open-problems.md` §1.4 (M27), `docs/language-bg.md`
  §14.3.10, `docs/language-en.md` §14.3.10.

### verify — M26: recv2/select2_wait като блокиращи операции
- **Тиша дупка затворена.** `chan_recv2` блокира точно като `chan_recv`
  (`while len==0 && !closed` върху `not_empty`), но нямаше никаква
  wait-for проверка — recv2 без producer минаваше тихо, а виси на runtime.
  Сега получава същата проверка като recv (последователни/producer/
  „recv без send").
- **select2_wait.** Блокира до елемент в който и да е от двата канала
  (или докато и двата се затворят — връща `(3,0)` веднага).
  `wf_chan_eventual` смята наличните елементи: буферирани (parent
  send-ове − recv-ове) + капацитет на producer-ите (нула след close —
  send върху затворен канал връща -1) − recv кредити. Има ли елемент от
  която и да е страна — **ДОКАЗАНО** („select — има producer"); няма ли
  и нищо не е неизвестно — **ОБРОЧЕНО** („select без producer").
- **Неизвестната страна на select-а.** Кой канал е консумиран не се
  знае: и двата канала се таксуват (`n_recv++`) и се маркират
  `wf_unknown`/`wf_sel_unknown` — следващи твърдения, зависещи от точното
  изцеждане, мълчат; ОБРОЧЕНО посоките остават звукови (данъкът е
  консервативен).
- **Затворен канал.** `chan_recv` върху затворен канал не блокира
  (връща 0 или буфериран елемент) — досега даваше фалшиво ОБРОЧЕНО
  („recv без send"); вече мълчи, но консумацията се брои (без това
  изцеден затворен канал изглеждаше удовлетворим за select — фалшиво
  ДОКАЗАНО, пазено от `lp6_select_drained_bad`).
- **Неблокиращите форми.** `chan_select2`, `chan_try_recv`,
  `chan_recv_timeout`, `chan_select2_timeout` не блокират — без
  твърдения; в worker-и остават complex (опционалната консумация чупи
  кредитната сметка). `recv2` в worker е броим recv.
- Оракъл: `examples/verify/select_wait.baga` (4 ДОКАЗАНО select,
  2 ОБРОЧЕНО select — и двата висят на runtime; recv след select —
  честно мълчание); адверсариални калъфи `lp6_recv2_dead_bad`,
  `lp6_select_dead_bad`, `lp6_select_drained_bad` в
  `examples/verify/lp6_hunt.baga`.
- Докс: `docs/thesis-open-problems.md` §1.4 (M26), `docs/language-bg.md`
  §14.3.9, `docs/language-en.md` §14.3.9.
...
- **Нов оператор `>>>`.** Логическо отместване надясно: знаковият бит се
  ...
  като при `>>`/`<<`. `-5 >>> 1 == 9223372036854775805`,
  `(-1) >>> 1 == 9223372036854775807`, `INT64_MIN >>> 63 == 1`; за `n >= 0`
  съвпада с `>>`. Затваря „logical shift" от `docs/thesis-open-problems.md`
  §2.2 (останаха: пълен bit-blast, wrap като стойност, променлив XOR,
  символни маски без знак, ashr върху i32).
- **Двата бекенда + self.** C бекендът емитува `baga_lshr_i64` (C `>>` върху
  отрицателни е имплементационно дефиниран); LLVM — `and 63` + `lshr`;
  self компилаторът — същият helper (`c2_mem.baga`/`c2_emit_expr.baga`).
  Пинат е и в `include/baga.h`.
- **Генерично цепене.** `>>>` в типова позиция се цепи на три затварящи `>`
  (C++11 стил): `Vec<Vec<Vec<i64>>>` се парсва коректно — `gt_pending` вече
  е брояч, не флаг; self лексерът/парсерът цепят токена по същия начин.
- **Верификаторът** записва `n >>> k` като собствен запис (`is_lshr`,
  броячът k) и инжектира честни аксиоми: безусловно `0 <= q <= 2^(64-k)-1`
  (знаковият бит е изчистен); `n >= 0` ⇒ floor обвивката на `>>`;
  `n <= -1` ⇒ `q >= 2^(63-k)`. Честни граници: точната връзка
  `q = floor((n + 2^64)/2^k)` за `n < 0` не е линейна по ℤ-променливите
  (`2^64` не се събира в i64); горна граница точно `INT64_MAX` (k=1) не се
  доказва — отрицанието ѝ изисква `INT64_MAX+1`, което прелива и FM ядрото
  честно отговаря „не мога да реша" (обща граница, не е нова).
- Оракули: `examples/verify/lshr_bounds.baga`; адверсариални калъфи
  `lp6_lshr_neg_bad`/`lp6_lshr_floor_bad` (LP6 батерията — пазят аксиомите
  на `>>` от преизползване за `>>>`); runtime паритет `tests/lshr_test.baga`
  (19 проверки, C + LLVM + self емисия).
- Докс: `docs/thesis-open-problems.md` §2 (M23), `docs/language-bg.md`
  §9.1/§14.3.6, `docs/language-en.md`, `docs/theory-bg.md` §7.

### verify + език — M22: signed right shift (аритметично отместване, floor)
- **Soundness фикс.** `n >> k` се записваше като C-trunc деление, чиято
  аксиома `2^k·q >= n` за `n <= 0` е **лъжлива** под ashr семантиката
  (и двата бекенда: LLVM `ashr`; gcc/clang `>>` де факто) — потенциално
  фалшиво ДОКАЗАНО (напр. `2*output - n >= 0` при `n <= -1`). Сега shift-ът
  е собствен запис (`is_shr`, броячът k вместо делител) и се пропуска от
  div+mod сдвояването.
- **Floor аксиоми.** `n >= 0` ⇒ trunc==floor обвивка (`0 <= n - d·q <= d-1`);
  `n <= 0` ⇒ `q <= 0`, `q >= n`, `0 <= n - d·q <= d-1`; неизвестен знак —
  честно слабо. Константно `n` се пресмята точно (`(-5) >> 1 == -3`).
- **Пината семантика на езика.** Броячът се маскира (`b & 63`) — не UB за
  отрицателни/≥64; `>>` е аритметично (floor; sign-fill). C бекендът емитува
  `baga_ashr_i64`/`baga_shl_i64` (C `>>` върху отрицателни е
  имплементационно дефиниран); LLVM — `and 63` + `ashr`/`shl`; self
  компилаторът — същото (`c2_mem.baga`/`c2_emit_expr.baga`).
- Оракули: `examples/verify/shr_floor.baga`; адверсариален калъф
  `lp6_shr_floor_bad` (LP6 батерията); runtime паритет `tests/ashr_test.baga`
  (21 проверки, C + LLVM + self емисия).
- Докс: `docs/thesis-open-problems.md` §2 (M22 — signed shift затворен),
  `docs/language-bg.md` §9.1/§14.3.5, `docs/language-en.md`.
